...
def get_list_preset_all():
    return get_list(name_collection)


def get_list_preset_method(opcode):
    all_preset = get_list(name_collection)
    for item_preset in all_preset:
        if item_preset['method'] == opcode:
            return item_preset['value']
